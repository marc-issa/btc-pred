...
def should_trade(prediction, market, elapsed_s, chainlink_price, price_to_beat, cal_data=None):
    ...
    if (market_price >= get_config("poly_slam_entry", config.POLY_SLAM_ENTRY)
            and market_price <= get_config("poly_momentum_max_buy", config.POLY_MOMENTUM_MAX_BUY)
            and elapsed_s >= get_config("poly_slam_min_elapsed", config.POLY_SLAM_MIN_ELAPSED)
    ...
    if (market_price >= get_config("poly_momentum_entry", config.POLY_MOMENTUM_ENTRY)
            and market_price <= get_config("poly_momentum_max_buy", config.POLY_MOMENTUM_MAX_BUY)
    ...
    if chainlink_price and price_to_beat and atr > 0:
        ...
        if betting_against and distance_atr > get_config("momentum_atr_threshold", config.MOMENTUM_ATR_THRESHOLD):
            return None, edge_val, 0
...
def calculate_bet_size(confidence, edge_val, balance, loss_streak=0, cal_data=None):
    ...
    if loss_streak >= get_config("consecutive_loss_limit", config.CONSECUTIVE_LOSS_LIMIT, cast=int):
        return min_bet
...
def check_early_exit(current_trade, market, remaining_s, prediction=None):
    ...
    low_volume = volume < get_config("low_volume_threshold", config.LOW_VOLUME_THRESHOLD)
    high_volume = volume >= get_config("high_volume_threshold", config.HIGH_VOLUME_THRESHOLD)
    ...
    if exit_min <= remaining_s <= exit_max:
        ...
        if max_profit > 0 and unrealized_pnl > 0:
            ...
            take_profit_thresh = get_config("early_exit_profit_pct", config.EARLY_EXIT_PROFIT_PCT)
            if conviction >= get_config("conviction_hold_threshold", config.CONVICTION_HOLD_THRESHOLD):
    ...
    if market_ratio >= get_config("market_agree_hold", config.MARKET_AGREE_HOLD):
        return False, None

    # ── Market disagrees (ratio < threshold) → check volume + conviction ──
    if market_ratio < get_config("market_disagree_sell", config.MARKET_DISAGREE_SELL):
        # Market has collapsed against us
        if high_volume:
            # High volume = real money says we're wrong → SELL
            return True, "market_disagree"
        elif low_volume and conviction >= get_config("conviction_hold_threshold", config.CONVICTION_HOLD_THRESHOLD):
    ...
    if unrealized_pnl < 0 and abs(unrealized_pnl) >= bet_size * get_config("stop_loss_pct", config.STOP_LOSS_PCT):
        # But gate it: if conviction is high and volume is low, hold
        if low_volume and conviction >= get_config("conviction_hold_threshold", config.CONVICTION_HOLD_THRESHOLD):
            return False, None
...
def check_position_flip(current_trade, prediction, market, remaining_s):
    ...
    """
    if remaining_s < get_config("flip_min_remaining", config.FLIP_MIN_REMAINING):
    ...
    if unrealized_pnl >= 0 or abs(unrealized_pnl) < bet_size * get_config("flip_loss_pct", config.FLIP_LOSS_PCT):
        return False, 0, None, 0
...
def render(market, price_to_beat, prediction, chainlink_now, trade_history, current_trade, balance,
    ...
    # Volume label
    if vol >= get_config("high_volume_threshold", config.HIGH_VOLUME_THRESHOLD):
        vol_label = f"{GREEN}HIGH{RESET}"
    elif vol >= get_config("low_volume_threshold", config.LOW_VOLUME_THRESHOLD):
    ...
    if current_trade and current_trade["action"] not in ("skip",):
        ...
        if not current_trade.get("_early_exited"):
            ...
            # Market ratio color
            if mr >= get_config("market_agree_hold", config.MARKET_AGREE_HOLD):
                mr_color = GREEN
                mr_label = "AGREE"
            elif mr >= get_config("market_disagree_sell", config.MARKET_DISAGREE_SELL):
                mr_color = YELLOW
                mr_label = "WEAK"
            else:
                mr_color = RED
            ...
            conv_color = GREEN if conv >= get_config("conviction_hold_threshold", config.CONVICTION_HOLD_THRESHOLD) else (YELLOW if conv >= 0.3 else DIM)
        ...
        if poly_max_now >= get_config("poly_slam_entry", config.POLY_SLAM_ENTRY) and elapsed_s >= get_config("poly_slam_min_elapsed", config.POLY_SLAM_MIN_ELAPSED):
    ...
    loss_warn = ""
    if consecutive_losses >= get_config("consecutive_loss_limit", config.CONSECUTIVE_LOSS_LIMIT, cast=int):
    ...
    try:
        ...
        # Volume label
        vol_label = "HIGH" if vol >= get_config("high_volume_threshold", config.HIGH_VOLUME_THRESHOLD) else (
            "MED" if vol >= get_config("low_volume_threshold", config.LOW_VOLUME_THRESHOLD) else "LOW")
            ...
            with pred_lock:
                if bg_prediction["value"] is not None:
                    ...
                    if current_trade and prediction and "error" not in prediction:
                        ...
                        early_ok = (poly_max >= get_config("poly_slam_entry", config.POLY_SLAM_ENTRY)
                                    and elapsed_in_window >= get_config("poly_slam_min_elapsed", config.POLY_SLAM_MIN_ELAPSED))
