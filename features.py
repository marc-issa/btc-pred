...
def add_intracandle_features(df_5m, df_1m):
    ...
    # Extract minute 0 and minute 1 data
    m0 = df_1m[df_1m["minute_in_window"] == 0].set_index("window")
    m1 = df_1m[df_1m["minute_in_window"] == 1].set_index("window")

    # Only keep windows where both minute 0 and minute 1 exist
    common = m0.index.intersection(m1.index).intersection(df_5m.index)
    ...
    # Combined high/low for first 2 minutes
    m2_high = pd.concat([m0_high, m1_high], axis=1).max(axis=1)
    m2_low = pd.concat([m0_low, m1_low], axis=1).min(axis=1)
    m2_vol = m0_vol + m1_vol
    ...
    ic["ic_range_2m"] = (m2_high - m2_low) / (candle_open + 1e-10)
    ic["ic_body_2m"] = (m1_close - candle_open) / (m2_high - m2_low + 1e-10)
    ic["ic_vol_2m"] = m2_vol

    # Momentum within candle
    ic["ic_momentum"] = (m1_close - m0_close) / (candle_open + 1e-10)
    ic["ic_accel"] = ((m1_close - m0_close) - (m0_close - candle_open)) / (candle_open + 1e-10)

    # Where is price vs the range so far
    ic["ic_range_pos_2m"] = (m1_close - m2_low) / (m2_high - m2_low + 1e-10)

    # Volume trend
    ic["ic_vol_ratio"] = m1_vol / (m0_vol + 1e-10)

    # Reference close for price_gap calculation
    ic["ic_close"] = m1_close
